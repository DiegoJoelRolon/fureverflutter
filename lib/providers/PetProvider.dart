...
class PetProvider extends ChangeNotifier {
  ...
  List<PetPost> get dogs =>
      _pets.where((p) => p.species == 'Perro' && p.adoptedStatus == 'Disponible').toList();

  List<PetPost> get cats =>
      _pets.where((p) => p.species == 'Gato' && p.adoptedStatus == 'Disponible').toList();

  List<PetPost> get puppies =>
      _pets.where((p) => p.ageGroup == 'Cachorro' && p.adoptedStatus == 'Disponible').toList();

  List<PetPost> get others =>
      _pets.where((p) => p.species == 'Otro' && p.adoptedStatus == 'Disponible').toList();
  ...
  List<PetPost> get filteredPets {
    return _pets.where((pet) {
      ...
      final matchesSpecies = _speciesFilter == 'Todas' || pet.species == _speciesFilter;
      final matchesGender  = _genderFilter == 'Todos'  || pet.gender == _genderFilter;
  ...
  void onSearchChanged(String q)       { _searchQuery = q;   notifyListeners(); }
  void onSpeciesChanged(String s)      { _speciesFilter = s;  notifyListeners(); }
  void onGenderChanged(String g)       { _genderFilter = g;   notifyListeners(); }
  void onSizeChanged(String s)         { _sizeFilter = s;     notifyListeners(); }
  void onAgeChanged(String a)          { _ageFilter = a;      notifyListeners(); }
  void clearFilters() {
    _searchQuery = '';
  ...
  Future<String> checkMyRequestStatus(String petId) async {
    ...
    final snap = await _db.collection('adoptionRequests')
  ...
  Future<void> sendAdoptionRequest(PetPost pet) async {
    ...
    final name = '${doc.data()?['name'] ?? ''} ${doc.data()?['lastname'] ?? ''}'.trim();
  ...
  Future<void> acceptRequest(AdoptionRequest request) async {
    ...
    batch.update(_db.collection('adoptionRequests').doc(request.id), {'status': 'accepted'});
  ...
  Future<void> rejectRequest(AdoptionRequest request) async {
    await _db.collection('adoptionRequests').doc(request.id).update({'status': 'rejected'});
  }
